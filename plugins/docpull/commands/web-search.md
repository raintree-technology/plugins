---
description: Search fetched web-source Markdown by regex and pull surrounding context for the best hits. Optionally restrict to one source alias.
...
- Don't call `ensure_docs` from this command. If the source isn't cached, send the user to `/web-add` instead - auto-fetching from a search command surprises people.
